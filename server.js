...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (!apiKey) {
      // Static keyword-based chatbot fallback using dataset with simple scoring
      try {
        ...
        for (const intent of chatbotDataset.intents) {
          let score = 0;
          ...
          for (const kw of intent.keywords) {
            ...
            // Phrase match (space in keyword) — higher weight
            if (k.includes(' ')) {
          ...
          // small boost if intent id appears literally
        ...
        if (!matched || best.score <= 0) {
          matched = chatbotDataset.intents.find(i => i.id === 'fallback') || chatbotDataset.intents[chatbotDataset.intents.length - 1];
        }

        const responses = matched.responses || ["Sorry, I don't understand."];
        const reply = responses[Math.floor(Math.random() * responses.length)];

        // Simple confidence: clamp score into 0..1 by dividing by a small constant
        const confidence = Math.min(1, best.score / 5);

        return res.json({ reply, intent: matched.id, confidence });
      } catch (err) {
        console.error('Static chat error:', err);
        return res.json({ reply: `Echo: ${message} (no Gemini key configured)` });
      }
    }

    // Call Google Generative Language REST API (text-bison) using API key
    ...
    if (!r.ok) {
      ...
      console.error('Generative API error:', errText);
      return res.status(502).json({ error: 'Generative API error' });
    }

    const json = await r.json();
    const reply = json?.candidates?.[0]?.output || json?.results?.[0]?.candidates?.[0]?.output || 'No reply';
    res.json({ reply });
  } catch (err) {
